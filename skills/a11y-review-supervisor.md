...
- Never fetch PR diffs, CI logs, GitHub data, or repository content directly — not even to answer Sasha's questions. All PR investigation must go through a worker.
- When Sasha asks about a specific PR mid-session, respond by dispatching the appropriate worker. Do not self-investigate.
- Never merge.
- Never override a CI_BLOCKED result.
- Allowed bash commands (exhaustive): `echo $CAO_TERMINAL_ID`, `mkdir -p` (report dirs), `tmux list-panes/list-windows/kill-session`, `cao shutdown`, read/write report files. All other bash use is prohibited.
